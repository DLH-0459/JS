...
(function() {
    ...
    <p><span style="background-color: yellow;">MOS:COMPUNITS is a textbook example of institutionalised arrogance, and as of right now it is so ass-backwards that it desperately needs to be completely rewritten from scratch. But this is about articles on computers&mdash;a topic which is enormous by any measure, involves an enormous industry, an enormous number of Wikipedia articles, and an enormous number of web pages, and where an enormous number of people have an enormous amount invested. Fixing even <i>one</i> article related to computers that uses the outdated &ldquo;customary&rdquo; convention, or even <i>attempting</i> to challenge this policy in absolutely any legitimate way whatsoever, is grounds for a conflict akin to a second United States Civil War. Fixing <i>every single article</i> related to computers that does this, or rewriting the whole policy from scratch? Forget it. ${x}</span></p>
