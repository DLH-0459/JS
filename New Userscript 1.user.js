...
(function() {
    ...
    <hr />
    <p><span style="background-color: yellow;">*&nbsp;Some very important policies in certain areas are really, really, really bad;** a good example is <a href="https://en.wikipedia.org/wiki/MOS:COMPUNITS">MOS:COMPUNITS</a>, a policy that governs articles on computers by effectively forcing users to preserve ambiguous usage when &ldquo;reliable&rdquo; sources use the obsolete &ldquo;customary&rdquo; convention of using SI prefixes (&ldquo;KB,&rdquo; &ldquo;MB,&rdquo; &ldquo;GB&rdquo; etc.) for binary (power of 1,024) values, while effectively prohibiting the use of clearer IEC prefixes (&ldquo;KiB,&rdquo; &ldquo;MiB,&rdquo; &ldquo;GiB&rdquo; etc.) unless the majority of sources use them. This policy in its current form is so absolutely horrible that it desperately needs a complete rewrite. But this is about articles on computers&mdash;an enormous topic, involving an enormous industry, an enormous number of web pages, and an enormous number of Wikipedia articles, and one where an enormous number of people have an enormous amount invested. Fixing even one article that uses the outdated customary convention or bringing just one paragraph of this policy up to modern standards are both grounds for a major conflict akin to a second United States Civil War. Fixing <i>every single article</i> on Wikipedia that does this <i>and</i> rewriting the whole policy from scratch? Forget it. ${desc_x}</span></p><br />
