...
(function() {
    ...
    let desc_x = `Your right to edit Wikipedia ends at the nose of some wack gatekeeper who is more willing to waste everyone&rsquo;s time and effort in edit wars than you will ever be. If you don&rsquo;t have near-unlimited time and persistence, you won&rsquo;t (and can&rsquo;t) win, no matter how correct you are.`;
    ...
    <hr />
    <p><span style="background-color: yellow;">*&nbsp;Some very important policies in certain areas are really, really, really bad;** a good example is <a href="https://en.wikipedia.org/wiki/MOS:COMPUNITS">MOS:COMPUNITS</a>, a policy that governs articles on computers by effectively forcing users to preserve ambiguous usage when &ldquo;reliable&rdquo; sources use the obsolete &ldquo;customary&rdquo; convention of using SI prefixes (&ldquo;KB,&rdquo; &ldquo;MB,&rdquo; &ldquo;GB,&rdquo; etc.) for binary (power of 1,024) values, while effectively prohibiting the use of clearer IEC prefixes (&ldquo;KiB,&rdquo; &ldquo;MiB,&rdquo; &ldquo;GiB,&rdquo; etc.) unless the majority of sources use them. This policy in its current form is so absolutely awful that it desperately needs to be completely rewritten. But this is about articles on computers&mdash;an enormous topic, involving an enormous industry, an enormous number of web pages, and an enormous number of Wikipedia articles, and one where an enormous number of people have an enormous amount invested. Fixing even one article that uses the outdated customary convention or bringing just one paragraph of this policy up to modern standards in any way are both grounds for a major conflict akin to a second United States Civil War. Fixing <i>every single article</i> on Wikipedia that does this <i>and</i> rewriting the whole policy from scratch? Forget it. ${desc_x}</span></p><br />

    <p><span style="background-color: yellow;">**&nbsp;Explanations:</span></p>

    <p><span style="background-color: yellow;">MOS:COMPUNITS claims that the SI prefixes are &ldquo;ambiguous in usage.&rdquo; While that is true among companies and many people, it is not true among the standards published by standards bodies. The SI prefixes mean powers of 1,000 and <i>not powers of 1,024</i>, while the IEC binary prefixes (KiB, MiB, GiB, TiB, etc.) mean powers of 1,024. This has been the internationally-accepted position since 1998, when the IEC first released their standard on the binary prefixes, and (most) competent experts in computing will use these terms correctly and support their correct usage.</span></p>
    ...
    <ol>
        ...
        <li>
            <p><span style="background-color: yellow;"><strong>Appeal to consequences:</strong></span></p>
