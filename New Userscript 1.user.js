...
(function() {
    ...
    <p><span style="background-color: yellow;">MOS:COMPUNITS is a textbook example of institutionalised arrogance, and as of right now it is so ass-backwards that it desperately needs to be completely rewritten from scratch. But this policy is about articles on computers&mdash;a massive topic, involving a massive industry, a massive number of Wikipedia articles, and a massive number of web pages, and where a massive number of people have a massive amount invested. Fixing even one article on Wikipedia that uses the outdated customary convention or section of this policy by yourself is grounds for a conflict akin to a second United States Civil War. Fixing <i>every single article</i> on Wikipedia that does this or rewriting the whole policy from scratch? Forget it. ${x}</span></p>
