...
(function() {
    ...
    <p><span style="background-color: yellow;">MOS:COMPUNITS is a textbook example of institutionalised arrogance, and it is so ass-backwards that it desperately needs a complete rewrite from scratch. But this policy is about articles on computers&mdash;a topic that is massive in scale, involves a massive industry, a massive number of Wikipedia articles, and a massive number of web pages, and where a massive number of people have a massive amount invested. Fixing even a single section of this policy by yourself is grounds for a conflict akin to a second United States Civil War. Rewriting the whole thing from scratch? Forget it. <i>${x}</i></span></p>
