...
(function() {
    ...
    <ol>
        ...
        <li>
            ...
            <p>Bureaucracy and edit wars are won by those who are most familiar with years of rules, not by those who are the most correct. And even if someone who goes into a fight is completely unable to wield Wikipedia&rsquo;s policies and years and years of arbitration and bureaucracy as weapons, they can still win in the end&mdash;again, not because they were the most correct, but because they <i>outlasted</i> everyone else, regardless of how right or wrong they were.</p>
        </li>
    </ol>
    <hr />
    <p>*&nbsp;Some very important policies in certain areas, even in STEM topics, are really, really bad.** A good example is <a href="https://en.wikipedia.org/wiki/MOS:COMPUNITS">MOS:COMPUNITS</a>, a policy that governs articles on computers by effectively forcing users to use the obsolete &ldquo;customary&rdquo; convention of using SI prefixes (&ldquo;KB,&rdquo; &ldquo;MB,&rdquo; &ldquo;GB,&rdquo; etc.) for binary (power of 1,024) values when the majority of &ldquo;reliable&rdquo; sources do so, while effectively prohibiting the use of the modern terminology (SI prefixes for powers of 1,000) or clearer IEC prefixes (&ldquo;KiB,&rdquo; &ldquo;MiB,&rdquo; &ldquo;GiB,&rdquo; etc.) unless the majority of sources use them that way. This policy is so absolutely awful that it desperately needs a complete rewrite from scratch. But this is the policy governing units of bits and bytes&mdash;as such, it affects thousands of articles across an enormous topic, involving an enormous industry, and where an awful lot of people have an awful lot invested. Fixing even one article that uses the &ldquo;customary&rdquo; convention or editing just one paragraph of this policy in any way are both grounds for a major conflict akin to a second United States Civil War. Fixing <i>every single article</i> on Wiki that does this <i>and</i> rewriting the whole policy from scratch? Forget it. ${desc_x}</p><br />
