...
(function() {
    ...
    <hr />
    <p><span style="background-color: yellow;">*&nbsp;Some very important policies in certain areas are really, really, really bad, even in STEM topics where Wikipedia might otherwise be fantastic at:</span>
    ...
    <p><span style="background-color: yellow;">All told, MOS:COMPUNITS in its current form is so thoroughly broken that nothing short of a complete rewrite will fix it. But this is about articles on computers&mdash;an enormous topic, involving an enormous industry, and one where an enormous number of people have an enormous amount invested. Fixing even one article on Wikipedia that uses the outdated customary convention or editing just one section of this policy in absolutely any legitimate way are both grounds for a major conflict akin to a second United States Civil War. Fixing <i>every single article</i> on Wikipedia that does this <i>and</i> rewriting the whole policy from scratch? Forget it. ${desc_x}</span></p>

    <p><span style="background-color: yellow;"><b>Wikipedia has always been infamous for being unreliable, since allowing anyone to edit inherently makes it vulnerable to vandalism and disruptive editing, which constantly requires editors to make corrections. But another part of this is that, ironically, many of their policies <i>prevent</i> factual corrections.</b></span></p>
